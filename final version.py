...
class QRCodeGeneratorApp:
    ...
    def generate_random_qr(self):
        ...
        print(qr_data_list,qr_labeling_list)
        if qr_data_list:
    ...
    def generate_qr_to_pdf(self, file_path, qr_data_list, qr_labeling_list=None):
        ...
        qr_per_row = 13 if qr_size == 10 else (7 if qr_size == 20 else 5)
        rows_per_page = 15 if qr_size == 10 else (12 if qr_size == 20 else 6)
        ...
        for i, (qr_data, qr_labeling) in enumerate(zip(qr_data_list, qr_labeling_list or [""] * len(qr_data_list))):
            qr = qrcode.QRCode(
                version=None,
            ...
            img_path = f"temp_qr_{qr_size}_{i}.png"
            img.save(img_path)
            ...
            if qr_counter >= qr_per_row:
                ...
                if y_start < 10 * mm or num_rows >= rows_per_page:
                    self.add_markers(c, page_width, page_height, x_start, y_start, qr_row_gap, qr_size, mm, qr_counter, num_rows, rows_per_page)
        ...
        # Add markers if last page is not full
        if qr_counter > 0 or num_rows < rows_per_page:
            self.add_markers(c, page_width, page_height, x_start, y_start, qr_row_gap, qr_size, mm,qr_counter, num_rows, rows_per_page)
    ...
    def add_markers(self, c, page_width, page_height, x_start, y_start, qr_row_gap, qr_size, mm, qr_counter, num_rows, rows_per_page):
        ...
        if num_rows == rows_per_page:
            # Page is full, place bottom markers at 5mm from bottom edge
            c.line(x_margin, y_margin, x_margin + 5 * mm, y_margin)
            c.line(x_margin, y_margin, x_margin, y_margin + 5 * mm)

            c.line(page_width - x_margin, y_margin, page_width - x_margin - 5 * mm, y_margin)
            c.line(page_width - x_margin, y_margin, page_width - x_margin, y_margin + 5 * mm)
        else:
            # Page is not full, place bottom markers at 5mm from last QR row
            c.line(x_margin, bottom_left_y, x_margin + 5 * mm, bottom_left_y)
            c.line(x_margin, bottom_left_y, x_margin, bottom_left_y + 5 * mm)
